...
class TestDagValidation:
    def test_valid_linear_dag(self):
        ...
        nodes = [
            DagNode(name="a", agent=a, depends_on=set()),
    ...
    def test_valid_diamond_dag(self):
        ...
        nodes = [
            DagNode(name="a", agent=agents["a"], depends_on=set()),
    ...
    def test_detects_cycle(self):
        ...
        with pytest.raises(ValueError, match="cycle"):
            DagAgent(name="test", dag_nodes=nodes, sub_agents=[a, b])

    def test_unknown_dependency(self):
        a = _make_stub_agent("a")
        nodes = [DagNode(name="a", agent=a, depends_on={"nonexistent"})]
        with pytest.raises(ValueError, match="Unknown dependency"):
            DagAgent(name="test", dag_nodes=nodes, sub_agents=[a])

    def test_single_node(self):
        a = _make_stub_agent("a")
        nodes = [DagNode(name="a", agent=a, depends_on=set())]
        dag = DagAgent(name="test", dag_nodes=nodes, sub_agents=[a])
        assert len(dag.dag_nodes) == 1

    def test_self_cycle(self):
        a = _make_stub_agent("a")
        nodes = [DagNode(name="a", agent=a, depends_on={"a"})]
        with pytest.raises(ValueError, match="cycle"):
            DagAgent(name="test", dag_nodes=nodes, sub_agents=[a])
